...
export default class HTTPMCPPlugin extends Plugin {
	...
	async startServer() {
		...
		if (!this.settings.apiKey) {
			new Notice('Please set an API key in settings before starting the server');
			return;
		}
	...
	private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
		...
		try {
			...
			console.log('MCP Request:', request.method, request.params, 'ID:', request.id);
			...
			case 'tools/list':
				return {
					tools: [
						...
						},
						{
							...
							inputSchema: {
								...
								properties: {
									...
									context_length: { type: 'number', description: 'Context length', default: 100 }
								},
								required: ['query']
							}
						},
						{
							name: 'patch_content',
							description: 'Patch content at specific locations (headings, blocks, frontmatter)',
							inputSchema: {
								type: 'object',
								properties: {
									filepath: { type: 'string', description: 'File path' },
									operation: { type: 'string', enum: ['append', 'prepend', 'replace'], description: 'Operation type' },
									target_type: { type: 'string', enum: ['heading', 'block', 'frontmatter'], description: 'Target type' },
						...
						{
							name: 'delete_file',
							description: 'Delete a file from the vault (requires confirmation)',
							inputSchema: {
								type: 'object',
								properties: {
	...
	private async callTool(toolName: string, args: any): Promise<any> {
		switch (toolName) {
			...
			case 'patch_content':
				...
				return { content: [{ type: 'text', text: 'Content patched successfully' }] };
			case 'simple_search':
				const searchResults = await this.simpleSearch(args.query, args.context_length);
	...
	private async appendContent(filepath: string, content: string): Promise<void> {
	...
	private async patchContent(params: any): Promise<void> {
		...
		if (target_type === 'heading') {
			...
			const restOfContent = fileContent.substring(headingIndex + match[0].length);
			...
			const sectionContent = fileContent.substring(headingIndex + match[0].length, sectionEnd);
			...
			fileContent = fileContent.substring(0, headingIndex + match[0].length) + 
						  newSectionContent + 
	...
	private async simpleSearch(query: string, contextLength: number = 100): Promise<any[]> {
